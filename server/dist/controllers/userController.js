...
export const searchUsers = async (req, res) => {
    try {
        ...
        const query = { deleted: { $ne: true } }; // exclude soft-deleted users
        if (username)
            query.login = new RegExp(username, "i"); // case-insensitive search
        if (location)
            query.location = new RegExp(location, "i");
        if (blog)
            query.blog = new RegExp(blog, "i");
        if (bio)
...
export const getFriends = async (req, res) => {
    try {
        ...
        const friends = following
            .filter((f) => followerLogins.has(f.login))
            .map((f) => f.login);
        // Save into DB
        user.followers_list = followers.map((f) => f.login);
        user.following_list = following.map((f) => f.login);
